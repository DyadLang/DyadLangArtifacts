...
# - Artifact tarballs must exist in gen/tarballs/ directory
...
function generate_release_notes(tarballs::Vector{String}, version::String)
    notes = """
    # DyadLangArtifacts.jl $version
    
    This release contains the dyad-lang repository artifacts.
    ...
    path = dyadlang_artifact_dir("artifact_name")
    
    # Get attribution information
    attribution = DyadLangArtifacts.get_attribution("artifact_name")
    
    # List all available artifacts
    artifacts = DyadLangArtifacts.list_artifacts()
    ```
    
    ## Attribution
    
    Each artifact includes an `ATTRIBUTION.md` file with complete source and commit information.
    The dyad-lang repository is available at: https://github.com/dyad-lang/dyad-lang
    """
...
function create_release()
    ...
    if isempty(tarballs)
        println("❌ No tarball files found in $TARBALLS_DIR")
    ...
    cmd = `gh release create $version --repo $repo --title "DyadLangArtifacts.jl $version" --notes-file $notes_file`
    run(cmd)
